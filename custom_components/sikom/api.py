...
class SikomApi:
    ...
    def __init__(self, hass, username: str, password: str) -> None:
        ...
        # Oppdages automatisk ved første oppdatering
        self.gateway_id: Optional[str] = None

        # Throttle for AppView-refresh (ikke hamre API oftere enn ca. hvert 5. minutt)
    ...
    # ---------------------------
    # Hjelpefunksjoner for refresh
    # ---------------------------
    def _should_refresh(self) -> bool:
        """Minst 5 min mellom refresh + liten jitter (0–30 sek)."""
        jitter = timedelta(seconds=random.randint(0, 30))
        return datetime.utcnow() - self._last_refresh >= timedelta(minutes=5) + jitter

    async def async_refresh_gateway(self, *, force: bool = False) -> None:
        """Forsøk å trigge samme oppfriskning som appen gjør (AppView v4.0)."""
        ...
        try:
            await self._request("GET", f"AppView/v4.0/{self.gateway_id}")
            self._last_refresh = datetime.utcnow()
            _LOGGER.debug("Sikom AppView v4.0 refresh OK")
            return
        except Exception as exc:
            # Non-fatal: vi faller tilbake til vanlig polling / fallback
            _LOGGER.debug("Sikom AppView v4.0 refresh feilet (ikke kritisk): %s", exc)

        # Fallback – noen oppsett ser ut til å oppdatere når Device/All leses
        try:
            await self._request("GET", f"Device/All/{self.gateway_id}")
            self._last_refresh = datetime.utcnow()
            _LOGGER.debug("Sikom refresh via Device/All fallback OK")
        except Exception as exc:
            _LOGGER.debug("Device/All fallback feilet (ikke kritisk): %s", exc)

    # ---------------------------
    # Device discovery / properties
    # ---------------------------
    async def list_devices(self) -> List[Dict[str, Any]]:
        """Henter og parser enhetslisten fra /Device/All/."""
        ...
        # Sett gateway_id hvis vi finner én kandidat
        if self.gateway_id is None:
            gw_ids = [str(d["gateway_id"]) for d in unique_devices if d.get("gateway_id")]
            if gw_ids:
                self.gateway_id = gw_ids[0]
                _LOGGER.debug("Oppdaget gateway_id=%s fra device-listen", self.gateway_id)
    ...
    async def get_property_value(self, device_id: int, prop: str) -> Any:
        """Hent verdi for en property (bruker 'scalar_result' når tilgjengelig)."""
